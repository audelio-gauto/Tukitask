...
const DOC_LABELS: Record<string, string> = {
  ...
  // vehicle-prefixed keys: moto_registro_frente, auto_registro_frente, etc.
};

function docLabel(key: string): string {
  if (DOC_LABELS[key]) return DOC_LABELS[key];
  // e.g. "moto_registro_frente" → "Moto — Registro frente"
    ...
    <div className="min-h-[60vh] flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 w-full max-w-sm">
        ...
        <form onSubmit={submit} className="space-y-4">
            ...
            <input
              ...
              className={`w-full px-4 py-3 rounded-xl border text-sm outline-none transition-colors pr-12
                ${error
                  ...
                  : 'border-gray-200 bg-gray-50 focus:border-[#F5C518] focus:bg-white'
                }`}
            />
            <button
              type="button"
          ...
          {error && (
            <p className="text-red-600 text-xs text-center font-medium">Contraseña incorrecta</p>
          )}
          <button
            type="submit"
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-3">
      ...
      <div className="text-[11px] text-gray-400 space-y-0.5">
        ...
        {doc.rejection_reason && (
          <p className="text-red-600">Motivo rechazo: {doc.rejection_reason}</p>
        )}
      </div>
...
export default function DocumentosPage() {
  ...
  const fetchDocs = useCallback(async (currentTab: 'pending' | 'all') => {
    if (!token) return;
    setLoading(true);
    try {
  ...
  useEffect(() => {
    if (unlocked && token) fetchDocs(tab);
  }, [unlocked, token, tab, fetchDocs]);
  ...
  const filtered = docs.filter(d => roleFilter === 'all' || d.role === roleFilter);
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">📎 Verificación de Documentos</h1>
          <p className="text-gray-500 text-sm mt-1">Revisá y aprobá los documentos de conductores y técnicos</p>
        </div>
        <button
          onClick={() => fetchDocs(tab)}
        ...
        <div className="space-y-8">
          ...
          {tab === 'pending' ? (
            pending.length === 0 ? (
              ...
              <div>
                <p className="text-sm font-semibold text-gray-600 mb-3">{pending.length} documento{pending.length !== 1 ? 's' : ''} pendiente{pending.length !== 1 ? 's' : ''}</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {pending.map(doc => (
                    <DocCard key={doc.id} doc={doc} onUpdate={handleUpdate} />
                  ))}
                </div>
              </div>
            )
            ...
            <>
              {pending.length > 0 && (
                <section>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {pending.map(doc => <DocCard key={doc.id} doc={doc} onUpdate={handleUpdate} />)}
                  </div>
                </section>
              )}
              {approved.length > 0 && (
                <section>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {approved.map(doc => <DocCard key={doc.id} doc={doc} onUpdate={handleUpdate} />)}
                  </div>
                </section>
              )}
              {rejected.length > 0 && (
                <section>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {rejected.map(doc => <DocCard key={doc.id} doc={doc} onUpdate={handleUpdate} />)}
                  </div>
                </section>
              )}
              {filtered.length === 0 && (
